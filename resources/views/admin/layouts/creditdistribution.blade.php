...
@section('content')
    <div class="row mt-5 justify-content-center">
        <div class="col-12">
            <div class="card border-0 shadow-lg rounded-4">
                <div class="card-header bg-dark bg-gradient text-white rounded-top-4 d-flex flex-wrap align-items-center justify-content-between gap-2 px-4 py-3">
                    <h5 class="mb-0 d-flex align-items-center gap-2">
                        <i class="fa-solid fa-book"></i> Credit Distribution
                    </h5>
                    <a href="#" class="btn btn-success btn-sm rounded-pill shadow-sm"
                        data-bs-toggle="modal" data-bs-target="#addCreditModal">
                        ...
                        <table class="table align-middle mb-0 table-hover small">
                            <thead class="table-info">
                                <tr>
                                    ...
                                    <th class="text-black text-center">Subject Type</th>
                                    <th class="text-black text-center">Credit Hour</th>
                            ...
                            <tbody>
                                @forelse ($credits as $credit)
                                    <tr>
                                        ...
                                        <td class="text-center text-black fw-semibold">{{ ucfirst($credit->subject_type) }}</td>
                                        <td class="text-center text-black fw-semibold">{{ $credit->credit_hour }}</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center text-black-50">No credit distributions available.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="modal-content rounded-4 shadow border-0">
                <form action="{{ route('creditdistribution.store') }}" method="POST">
                    ...
                    <div class="modal-header bg-dark bg-gradient text-white rounded-top-4 py-2 px-3">
                        <h6 class="modal-title" id="addCreditModalLabel">
                    ...
                    <div class="modal-body bg-light py-3 px-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control form-control-sm rounded-3 shadow-sm"
                                id="credit_hour" name="credit_hour"
                                placeholder="Enter credit hour" required>
                        </div>
                    </div>
    ...
    @foreach ($credits as $credit)
        <div class="modal fade" id="editCreditModal-{{ $credit->id }}" tabindex="-1"
            aria-labelledby="editCreditModalLabel-{{ $credit->id }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content rounded-4 shadow border-0">
                    <form action="{{ route('creditdistribution.update', $credit->id) }}" method="POST">
                        ...
                        @method('PUT')
                        <div class="modal-header bg-dark bg-gradient text-white rounded-top-4 py-2 px-3">
                            <h6 class="modal-title" id="editCreditModalLabel-{{ $credit->id }}">
                        ...
                        <div class="modal-body bg-light py-3 px-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control form-control-sm rounded-3 shadow-sm"
                                    id="credit_hour-{{ $credit->id }}"
                                    name="credit_hour" value="{{ $credit->credit_hour }}" required>
                            </div>
                        </div>
                        <div class="modal-footer bg-light rounded-bottom-4 d-flex justify-content-end gap-2 py-2">
                            <button type="button" class="btn btn-outline-secondary btn-sm rounded-pill px-3" data-bs-dismiss="modal">
